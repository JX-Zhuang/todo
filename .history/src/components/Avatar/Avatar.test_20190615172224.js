...
describe('<Avatar/>', () => {
    ...
    it('renders Avatar and clicks', () => {
        ...
        MenuNext.find(MenuItem).at(0).simulate('click');

        const MenuAfterItemClicks = wrapper.find(Menu); 
        expect(MenuAfterItemClicks.props().open).toEqual(true);
        expect(MenuAfterItemClicks.props().anchorEl).toEqual(IconButton);  
    });
})